...
-(void)tc_invoke:(NSTimer*)sender;
{
	void(^block)(NSTimer*) = (__bridge void (^)(NSTimer *__strong))((__bridge void*)self);
	block(sender);
}
@end
...
{
	return [NSTimer timerWithTimeInterval:ti target:[block copy] selector:@selector(tc_invoke:) userInfo:nil repeats:yesOrNo];
}
+ (NSTimer*)scheduledTimerWithTimeInterval:(NSTimeInterval)ti repeats:(BOOL)yesOrNo block:(void(^)(NSTimer*))block;
{
	return [NSTimer scheduledTimerWithTimeInterval:ti target:[block copy] selector:@selector(tc_invoke:) userInfo:nil repeats:yesOrNo];
}
@end